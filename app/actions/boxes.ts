...
export async function purchaseBoxAction(formData: FormData) {
  ...
  // Added Qty and Rate for consistency with editing!
...
export async function editBoxPurchaseAction(formData: FormData) {
  ...
  const material_name = matData?.name || "Unknown Box";
  ...
  const old_desc = `Purchase - ${txn.reason} (${old_quantity} Boxes of ${material_name})`;
  const new_desc = `Purchase - ${new_supplier} (${new_quantity} Boxes of ${material_name})`;
